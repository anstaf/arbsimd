...
namespace detail {
    ...
    public:
        ...
        template <typename Other, std::enable_if_t<width==simd_traits<Other>::width, int> = 0>
        ...
        template <typename IndexImpl, std::enable_if_t<width==simd_traits<IndexImpl>::width, int> = 0>
        ...
        template <typename Index, std::enable_if_t<width==simd_traits<typename Index::simd_base>::width, int> = 0>
        ...
        template <typename T>\
        friend simd_impl<T> arbsimd::name(const simd_impl<T>& a);

        #define ARB_DECLARE_BINARY_ARITHMETIC_(name)\
        template <typename T>\
        friend simd_impl<T> arbsimd::name(const simd_impl<T>& a, simd_impl<T> b);\
        template <typename T>\
        friend simd_impl<T> arbsimd::name(const simd_impl<T>& a, typename simd_impl<T>::scalar_type b);\
        template <typename T>\
        friend simd_impl<T> arbsimd::name(const typename simd_impl<T>::scalar_type a, simd_impl<T> b);

        #define ARB_DECLARE_BINARY_COMPARISON_(name)\
        template <typename T>\
        friend typename simd_impl<T>::simd_mask arbsimd::name(const simd_impl<T>& a, simd_impl<T> b);\
        template <typename T>\
        friend typename simd_impl<T>::simd_mask arbsimd::name(const simd_impl<T>& a, typename simd_impl<T>::scalar_type b);\
        template <typename T>\
        friend typename simd_impl<T>::simd_mask arbsimd::name(const typename simd_impl<T>::scalar_type a, simd_impl<T> b);
        ...
        template <typename T>
        friend simd_impl<T> arbsimd::fma(const simd_impl<T> a, simd_impl<T> b, simd_impl<T> c);
    ...
    struct simd_cast_impl<simd_mask_impl<ImplTo>> {
        ...
        template <typename ImplFrom, std::enable_if_t<N==simd_traits<ImplFrom>::width, int> = 0>
...
constexpr auto width(const S a = S{}) {
    return S::width;
};

template <typename S, std::enable_if_t<is_simd<S>::value, int> = 0>
constexpr auto min_align(const S a = S{}) {
    return S::min_align;
};
