...
namespace testing {
namespace kernel_point_ica_current {
...
[[maybe_unused]] auto* _pp_var_ica_nA = pp->state_vars[0];\
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ica = _pp_var_ica_nA[i_];
        current_ = ica;
        _pp_var_vec_i[node_indexi_] = fma(_pp_var_weight[i_], current_, _pp_var_vec_i[node_indexi_]);
        _pp_var_ion_ca.current_density[ion_ca_indexi_] = fma(_pp_var_weight[i_], ica, _pp_var_ion_ca.current_density[ion_ca_indexi_]);
    }
}
...
extern "C" {
  arb_mechanism_interface* make_testing_point_ica_current_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = testing::kernel_point_ica_current::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = testing::kernel_point_ica_current::min_align_;
    result.init_mechanism = testing::kernel_point_ica_current::init;
    result.compute_currents = testing::kernel_point_ica_current::compute_currents;
    result.apply_events = testing::kernel_point_ica_current::apply_events;
    result.advance_state = testing::kernel_point_ica_current::advance_state;
    result.write_ions = testing::kernel_point_ica_current::write_ions;
    result.post_event = testing::kernel_point_ica_current::post_event;
    return &result;
  }}
