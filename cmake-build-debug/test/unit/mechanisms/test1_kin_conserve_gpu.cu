...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        t_1_ = a_9_*a_5_-a_4_*_pp_var_y[tid_];
        t_2_ = t_0_*a_9_;
        t_3_ = t_0_*_pp_var_y[tid_]-a_8_*t_1_;
        t_4_ = a_7_*a_0_-a_1_*a_6_;
        t_5_ = a_7_*a_2_-a_1_*_pp_var_b[tid_];
        t_6_ = t_4_*a_7_;
        t_7_ = t_4_*_pp_var_b[tid_]-a_6_*t_5_;
...
void mechanism_test1_kin_conserve_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_test1_kin_conserve_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_test1_kin_conserve_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_test1_kin_conserve_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_test1_kin_conserve_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_test1_kin_conserve_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
