...
[[maybe_unused]] auto* _pp_var_record_ex = pp->state_vars[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type ex = _pp_var_ion_x.reversal_potential[ion_x_indexi_];
        _pp_var_record_ex[i_] = ex;
    }
...
extern "C" {
  arb_mechanism_interface* make_testing_read_eX_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = testing::kernel_read_eX::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = testing::kernel_read_eX::min_align_;
    result.init_mechanism = testing::kernel_read_eX::init;
    result.compute_currents = testing::kernel_read_eX::compute_currents;
    result.apply_events = testing::kernel_read_eX::apply_events;
    result.advance_state = testing::kernel_read_eX::advance_state;
    result.write_ions = testing::kernel_read_eX::write_ions;
    result.post_event = testing::kernel_read_eX::post_event;
    return &result;
  }}
