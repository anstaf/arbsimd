...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        l_4_ =  -( -_pp_var_a1[tid_]+_pp_var_a0[tid_]);
        l_5_ =  0.;
        l_6_ =  1.0;
        l_7_ =  1.0;
        l_8_ =  1.0;
...
void mechanism_test_linear_state_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_test_linear_state_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_test_linear_state_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_test_linear_state_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_test_linear_state_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_test_linear_state_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
