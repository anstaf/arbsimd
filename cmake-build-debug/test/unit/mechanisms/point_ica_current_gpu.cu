...
auto* _pp_var_ica_nA __attribute__((unused)) = params_.state_vars[0];\
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ica = _pp_var_ica_nA[tid_];
        current_ = ica;
        ::arb::gpu::reduce_by_key(_pp_var_weight[tid_]*current_,_pp_var_vec_i, node_indexi_, lane_mask_);
        ::arb::gpu::reduce_by_key(_pp_var_weight[tid_]*ica,_pp_var_ion_ca.current_density, ion_ca_indexi_, lane_mask_);
    }
}
...
void mechanism_point_ica_current_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_point_ica_current_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_point_ica_current_gpu_advance_state_(arb_mechanism_ppack* p) {}

void mechanism_point_ica_current_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_point_ica_current_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_point_ica_current_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* stream_ptr) {
