...
auto* _pp_var_coeff __attribute__((unused)) = params_.parameters[0];\
...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        arb_value_type b_0_;
        b_0_ =  -_pp_var_coeff[tid_]*ica;
        _pp_var_cai[tid_] = _pp_var_cai[tid_]+b_0_*dt;
    }
}
...
void mechanism_linear_ca_conc_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_linear_ca_conc_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_linear_ca_conc_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_linear_ca_conc_gpu_write_ions_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_linear_ca_conc_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_linear_ca_conc_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
