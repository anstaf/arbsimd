...
auto& _pp_var_ion_y __attribute__((unused)) = params_.ion_states[1];\
auto* _pp_var_ion_y_index __attribute__((unused)) = params_.ion_states[1].index;\
//End of IFACEBLOCK
...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        auto ion_y_indexi_ = _pp_var_ion_y_index[tid_];
...
void mechanism_write_multiple_eX_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_write_multiple_eX_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_write_multiple_eX_gpu_advance_state_(arb_mechanism_ppack* p) {}

void mechanism_write_multiple_eX_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_write_multiple_eX_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_write_multiple_eX_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
