...
auto* _pp_var_e __attribute__((unused)) = params_.parameters[1];\
//End of IFACEBLOCK
    ...
    if(tid_<stream.n_streams) {
        ...
        for (auto p = begin; p<end; ++p) {
            if (p->mech_id==_pp_var_mechanism_id) {
                ...
                _pp_var_g[tid_] = _pp_var_g[tid_]+weight;
            }
        }
    }
}
...
void post_event(arb_mechanism_ppack params_) {
    ...
    if (tid_<_pp_var_width) {
        ...
        for (unsigned c = 0; c < _pp_var_n_detectors; c++) {
            ...
            if (t >= 0) {
                _pp_var_g[tid_] = _pp_var_g[tid_]+ 0.10000000000000001*t;
            }
        }
    }
}
...
void mechanism_post_events_syn_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_post_events_syn_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_post_events_syn_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_post_events_syn_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_post_events_syn_gpu_post_event_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_post_events_syn_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* stream_ptr) {
