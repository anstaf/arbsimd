    ...
    for (arb_size_type c = 0; c<ncell; ++c) {
        ...
        for (auto p = begin; p<end; ++p) {
            ...
            if (p->mech_id==_pp_var_mechanism_id) {
                _pp_var_g[i_] = max( 0., min(_pp_var_g[i_]+weight,  10.0));
            }
        }
    }
}
...
extern "C" {
  arb_mechanism_interface* make_testing_non_linear_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = testing::kernel_non_linear::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = testing::kernel_non_linear::min_align_;
    result.init_mechanism = testing::kernel_non_linear::init;
    result.compute_currents = testing::kernel_non_linear::compute_currents;
    result.apply_events = testing::kernel_non_linear::apply_events;
    result.advance_state = testing::kernel_non_linear::advance_state;
    result.write_ions = testing::kernel_non_linear::write_ions;
    result.post_event = testing::kernel_non_linear::post_event;
    return &result;
  }}
