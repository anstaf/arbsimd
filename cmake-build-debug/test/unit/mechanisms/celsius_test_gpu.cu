...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_c[tid_] = celsius;
    }
}

} // namespace

void mechanism_celsius_test_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_celsius_test_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_celsius_test_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_celsius_test_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_celsius_test_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_celsius_test_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
