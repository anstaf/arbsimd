...
namespace testing {
namespace kernel_test2_kin_diff {
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type t_16_, t_15_, t_13_, t_12_, t_11_, t_9_, t_7_, t_10_, p_2_, j_8_, j_4_, j_6_, f_2_, t_5_, j_7_, f_1_, j_2_, t_14_, j_3_, f_0_, t_4_, f, t_2_, t_6_, r, j_0_, p_0_, t_3_, j_5_, t_17_, t_8_, j_1_, p_1_, t_1_, t_0_;
        p_0_ = _pp_var_a[i_];
        t_0_ = _pp_var_a[i_];
        p_1_ = _pp_var_b[i_];
        t_1_ = _pp_var_b[i_];
        ...
        f =  2.0;
        r =  1.0;
        f_0_ = t_0_-(p_0_+ -2.0*(t_1_*(pow(t_0_,  2.0)*f)-t_2_*r)*dt);
        f_1_ = t_1_-(p_1_+ -1.0*(t_1_*(pow(t_0_,  2.0)*f)-t_2_*r)*dt);
        f_2_ = t_2_-(p_2_+(t_1_*(pow(t_0_,  2.0)*f)-t_2_*r)*dt);
        j_0_ =  1.0- -2.0*(t_1_*( 2.0*t_0_*f))*dt;
        j_1_ =  -( -2.0*(pow(t_0_,  2.0)*f)*dt);
        j_2_ =  -( -2.0* -r*dt);
        j_3_ =  -( -1.0*(t_1_*( 2.0*t_0_*f))*dt);
        j_4_ =  1.0- -1.0*(pow(t_0_,  2.0)*f)*dt;
        j_5_ =  -( -1.0* -r*dt);
        j_6_ =  -(t_1_*( 2.0*t_0_*f)*dt);
        j_7_ =  -(pow(t_0_,  2.0)*f*dt);
        j_8_ =  1.0- -r*dt;
...
extern "C" {
  arb_mechanism_interface* make_testing_test2_kin_diff_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = testing::kernel_test2_kin_diff::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = testing::kernel_test2_kin_diff::min_align_;
    result.init_mechanism = testing::kernel_test2_kin_diff::init;
    result.compute_currents = testing::kernel_test2_kin_diff::compute_currents;
    result.apply_events = testing::kernel_test2_kin_diff::apply_events;
    result.advance_state = testing::kernel_test2_kin_diff::advance_state;
    result.write_ions = testing::kernel_test2_kin_diff::write_ions;
    result.post_event = testing::kernel_test2_kin_diff::post_event;
    return &result;
  }}
