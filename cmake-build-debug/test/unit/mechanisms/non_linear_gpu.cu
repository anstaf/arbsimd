    ...
    if(tid_<stream.n_streams) {
        ...
        for (auto p = begin; p<end; ++p) {
            if (p->mech_id==_pp_var_mechanism_id) {
                ...
                _pp_var_g[tid_] = max( 0., min(_pp_var_g[tid_]+weight,  10.0));
            }
        }
    }
}
...
void mechanism_non_linear_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_non_linear_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_non_linear_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_non_linear_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_non_linear_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_non_linear_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* stream_ptr) {
