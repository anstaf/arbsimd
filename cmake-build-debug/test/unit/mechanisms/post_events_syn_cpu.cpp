...
namespace testing {
namespace kernel_post_events_syn {
...
static void post_event(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        for (auto c = 0; c < _pp_var_n_detectors; c++) {
            auto t = _pp_var_time_since_spike[offset_ + c];
            if (t >= 0) {
                _pp_var_g[i_] = _pp_var_g[i_]+ 0.10000000000000001*t;
            }
        }
    }
}
...
extern "C" {
  arb_mechanism_interface* make_testing_post_events_syn_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = testing::kernel_post_events_syn::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = testing::kernel_post_events_syn::min_align_;
    result.init_mechanism = testing::kernel_post_events_syn::init;
    result.compute_currents = testing::kernel_post_events_syn::compute_currents;
    result.apply_events = testing::kernel_post_events_syn::apply_events;
    result.advance_state = testing::kernel_post_events_syn::advance_state;
    result.write_ions = testing::kernel_post_events_syn::write_ions;
    result.post_event = testing::kernel_post_events_syn::post_event;
    return &result;
  }}
