...
auto* _pp_var_current_density __attribute__((unused)) = params_.parameters[0];\
...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ica = _pp_var_current_density[tid_];
...
void mechanism_fixed_ica_current_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_fixed_ica_current_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_fixed_ica_current_gpu_advance_state_(arb_mechanism_ppack* p) {}

void mechanism_fixed_ica_current_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_fixed_ica_current_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_fixed_ica_current_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
