...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        t_2_ = a_6_*a_3_-a_1_*_pp_var_h[tid_];
        t_3_ = a_4_*t_1_-t_0_*a_5_;
        t_4_ = a_4_*t_2_-t_0_*_pp_var_d[tid_];
        t_5_ = t_3_*a_4_;
        t_6_ = t_3_*_pp_var_d[tid_]-a_5_*t_4_;
        t_7_ = t_3_*a_6_;
        t_8_ = t_3_*_pp_var_h[tid_]-a_7_*t_4_;
...
void mechanism_test0_kin_conserve_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_test0_kin_conserve_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_test0_kin_conserve_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_test0_kin_conserve_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_test0_kin_conserve_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_test0_kin_conserve_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
