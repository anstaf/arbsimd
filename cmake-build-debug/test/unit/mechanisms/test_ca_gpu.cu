...
auto _pp_var_decay __attribute__((unused)) = params_.globals[0];\
auto _pp_var_cai0 __attribute__((unused)) = params_.globals[1];\
auto _pp_var_factor __attribute__((unused)) = params_.globals[2];\
...
void mechanism_test_ca_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_test_ca_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_test_ca_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_test_ca_gpu_write_ions_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_test_ca_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_test_ca_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
