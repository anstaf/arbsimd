...
auto* _pp_var_gNaTa_tbar __attribute__((unused)) = params_.parameters[0];\
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ina = _pp_var_gNaTa_tbar[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_h[tid_]*(v-ena);
        current_ = ina;
        conductivity_ = _pp_var_gNaTa_tbar[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_h[tid_];
...
void mechanism_NaTa_t_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_NaTa_t_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_NaTa_t_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_NaTa_t_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_NaTa_t_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_NaTa_t_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
