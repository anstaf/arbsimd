...
[[maybe_unused]] auto* _pp_var_gK_Tstbar = pp->parameters[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        _pp_var_m[i_] =  1.0/( 1.0+exp( -(v+ 10.0)* 0.052631578947368418));
        _pp_var_h[i_] =  1.0/( 1.0+exp( -(v+ 76.0)* -0.10000000000000001));
    }
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        mInf =  1.0/( 1.0+exp( -(v+ 10.0)* 0.052631578947368418));
        mTau =  0.34000000000000002+ 0.92000000000000004*exp(pow( -((v+ 81.0)* 0.016949152542372881),  2.0));
        hInf =  1.0/( 1.0+exp( -(v+ 76.0)* -0.10000000000000001));
        hTau =  8.0+ 49.0*exp(pow( -((v+ 83.0)* 0.043478260869565216),  2.0));
        a_0_ = qt* -1.0/mTau;
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type ik = 0;
        ik = _pp_var_gK_Tstbar[i_]*pow(_pp_var_m[i_],  4.0)*_pp_var_h[i_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gK_Tstbar[i_]*pow(_pp_var_m[i_],  4.0)*_pp_var_h[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_K_Tst_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_K_Tst::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_K_Tst::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_K_Tst::init;
    result.compute_currents = arb::bbp_catalogue::kernel_K_Tst::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_K_Tst::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_K_Tst::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_K_Tst::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_K_Tst::post_event;
    return &result;
  }}
