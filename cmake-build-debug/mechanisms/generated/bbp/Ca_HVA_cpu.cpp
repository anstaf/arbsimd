...
[[maybe_unused]] auto* _pp_var_gCa_HVAbar = pp->parameters[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        mAlpha =  0.20899999999999999*exprelr( -( 27.0+v)* 0.26315789473684209);
        mBeta =  0.93999999999999995*exp( -(v+ 75.0)* 0.058823529411764705);
        hAlpha =  0.000457*exp( -(v+ 13.0)* 0.02);
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type a_0_, mBeta, a_1_, mAlpha, ll2_, hBeta, ba_0_, ll0_, ba_1_, ll3_, ll1_, hAlpha;
        ll3_ =  0.;
        ll2_ =  0.;
        ll1_ =  0.;
        ll0_ =  0.;
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ica = _pp_var_gCa_HVAbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_]*(v-eca);
        current_ = ica;
        conductivity_ = _pp_var_gCa_HVAbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_Ca_HVA_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_Ca_HVA::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_Ca_HVA::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_Ca_HVA::init;
    result.compute_currents = arb::bbp_catalogue::kernel_Ca_HVA::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_Ca_HVA::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_Ca_HVA::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_Ca_HVA::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_Ca_HVA::post_event;
    return &result;
  }}
