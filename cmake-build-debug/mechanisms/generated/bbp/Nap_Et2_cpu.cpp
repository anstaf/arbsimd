...
[[maybe_unused]] auto* _pp_var_gNap_Et2bar = pp->parameters[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        _pp_var_m[i_] =  1.0/( 1.0+exp( -(v+ 52.600000000000001)* 0.21739130434782611));
        _pp_var_h[i_] =  1.0/( 1.0+exp((v+ 48.799999999999997)* 0.10000000000000001));
    }
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ina = _pp_var_gNap_Et2bar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_]*(v-ena);
        current_ = ina;
        conductivity_ = _pp_var_gNap_Et2bar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_Nap_Et2_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_Nap_Et2::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_Nap_Et2::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_Nap_Et2::init;
    result.compute_currents = arb::bbp_catalogue::kernel_Nap_Et2::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_Nap_Et2::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_Nap_Et2::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_Nap_Et2::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_Nap_Et2::post_event;
    return &result;
  }}
