...
auto _pp_var_ehcn __attribute__((unused)) = params_.globals[0];\
auto* _pp_var_m __attribute__((unused)) = params_.state_vars[0];\
auto* _pp_var_gIhbar __attribute__((unused)) = params_.parameters[0];\
//End of IFACEBLOCK
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ihcn = _pp_var_gIhbar[tid_]*_pp_var_m[tid_]*(v-_pp_var_ehcn);
        current_ = ihcn;
        conductivity_ = _pp_var_gIhbar[tid_]*_pp_var_m[tid_];
...
void mechanism_Ih_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_Ih_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Ih_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Ih_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_Ih_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_Ih_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
