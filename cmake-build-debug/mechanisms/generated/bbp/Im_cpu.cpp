...
[[maybe_unused]] auto* _pp_var_gImbar = pp->parameters[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        mAlpha =  0.0033*exp( 0.10000000000000001*(v+ 35.0));
        mBeta =  0.0033*exp( -0.10000000000000001*(v+ 35.0));
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type a_0_, qt, mBeta, mAlpha, ba_0_, ll0_, ll1_;
        ll1_ =  0.;
        ll0_ =  0.;
        qt =  2.952882641412121;
        mAlpha =  0.0033*exp( 0.10000000000000001*(v+ 35.0));
        mBeta =  0.0033*exp( -0.10000000000000001*(v+ 35.0));
        a_0_ = qt* -(mAlpha+mBeta);
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type ik = 0;
        ik = _pp_var_gImbar[i_]*_pp_var_m[i_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gImbar[i_]*_pp_var_m[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_Im_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_Im::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_Im::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_Im::init;
    result.compute_currents = arb::bbp_catalogue::kernel_Im::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_Im::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_Im::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_Im::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_Im::post_event;
    return &result;
  }}
