...
auto* _pp_var_gImbar __attribute__((unused)) = params_.parameters[0];\
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type ik = 0;
        ik = _pp_var_gImbar[tid_]*_pp_var_m[tid_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gImbar[tid_]*_pp_var_m[tid_];
