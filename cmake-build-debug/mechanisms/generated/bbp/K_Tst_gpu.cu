...
auto* _pp_var_gK_Tstbar __attribute__((unused)) = params_.parameters[0];\
...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_m[tid_] =  1.0/( 1.0+exp( -(v+ 10.0)* 0.052631578947368418));
        _pp_var_h[tid_] =  1.0/( 1.0+exp( -(v+ 76.0)* -0.10000000000000001));
    }
}
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type ik = 0;
        ik = _pp_var_gK_Tstbar[tid_]*pow(_pp_var_m[tid_],  4.0)*_pp_var_h[tid_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gK_Tstbar[tid_]*pow(_pp_var_m[tid_],  4.0)*_pp_var_h[tid_];
...
void mechanism_K_Tst_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_K_Tst_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_K_Tst_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_K_Tst_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_K_Tst_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_K_Tst_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
