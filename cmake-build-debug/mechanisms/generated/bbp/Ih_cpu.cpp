...
[[maybe_unused]] auto* _pp_var_gIhbar = pp->parameters[0];\
//End of IFACEBLOCK
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        mAlpha =  0.076517000000000002*exprelr((v+ 154.90000000000001)* 0.084033613445378144);
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type a_0_, mBeta, mAlpha, ba_0_, ll0_, ll1_;
        ll1_ =  0.;
        ll0_ =  0.;
        mAlpha =  0.076517000000000002*exprelr((v+ 154.90000000000001)* 0.084033613445378144);
        mBeta =  0.193*exp(v* 0.030211480362537763);
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ihcn = _pp_var_gIhbar[i_]*_pp_var_m[i_]*(v-_pp_var_ehcn);
        current_ = ihcn;
        conductivity_ = _pp_var_gIhbar[i_]*_pp_var_m[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_Ih_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_Ih::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_Ih::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_Ih::init;
    result.compute_currents = arb::bbp_catalogue::kernel_Ih::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_Ih::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_Ih::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_Ih::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_Ih::post_event;
    return &result;
  }}
