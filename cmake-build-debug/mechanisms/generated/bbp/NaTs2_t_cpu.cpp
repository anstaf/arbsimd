...
[[maybe_unused]] auto* _pp_var_gNaTs2_tbar = pp->parameters[0];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        mAlpha =  1.0920000000000001*exprelr( -(v+ 32.0)* 0.16666666666666666);
        mBeta =  0.74399999999999999*exprelr((v+ 32.0)* 0.16666666666666666);
        _pp_var_m[i_] = mAlpha/(mAlpha+mBeta);
        hAlpha =  0.089999999999999997*exprelr((v+ 60.0)* 0.16666666666666666);
        hBeta =  0.089999999999999997*exprelr( -(v+ 60.0)* 0.16666666666666666);
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ina = _pp_var_gNaTs2_tbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_]*(v-ena);
        current_ = ina;
        conductivity_ = _pp_var_gNaTs2_tbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_bbp_catalogue_NaTs2_t_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::bbp_catalogue::kernel_NaTs2_t::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::bbp_catalogue::kernel_NaTs2_t::min_align_;
    result.init_mechanism = arb::bbp_catalogue::kernel_NaTs2_t::init;
    result.compute_currents = arb::bbp_catalogue::kernel_NaTs2_t::compute_currents;
    result.apply_events = arb::bbp_catalogue::kernel_NaTs2_t::apply_events;
    result.advance_state = arb::bbp_catalogue::kernel_NaTs2_t::advance_state;
    result.write_ions = arb::bbp_catalogue::kernel_NaTs2_t::write_ions;
    result.post_event = arb::bbp_catalogue::kernel_NaTs2_t::post_event;
    return &result;
  }}
