...
void init(arb_mechanism_ppack params_) {
    ...
    PPACK_IFACE_BLOCK;
    if (tid_<n_) {
        _pp_var_cai[tid_] = _pp_var_minCai[tid_];
    }
}
...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ba_0_ = ( -10000.0*ica*_pp_var_gamma[tid_]/( 2.0*_pp_var_F*_pp_var_depth[tid_])- -_pp_var_minCai[tid_]/_pp_var_decay[tid_])/a_0_;
        ll0_ = a_0_*dt;
...
void mechanism_CaDynamics_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_CaDynamics_gpu_compute_currents_(arb_mechanism_ppack* p) {}

void mechanism_CaDynamics_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_CaDynamics_gpu_write_ions_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_CaDynamics_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_CaDynamics_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
