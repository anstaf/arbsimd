...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_m[tid_] =  1.0- 1.0/( 1.0+exp((v+ 43.0)* 0.125));
        _pp_var_h[tid_] =  1.0/( 1.0+exp((v+ 67.0)* 0.13698630136986301));
    }
}
...
void mechanism_Kd_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_Kd_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Kd_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Kd_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_Kd_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_Kd_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
