...
auto* _pp_var_qt __attribute__((unused)) = params_.state_vars[2];\
...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_qt[tid_] = pow( 2.2999999999999998, (celsius- 21.0)* 0.10000000000000001);
...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        mRat = _pp_var_qt[tid_]/( 5.0+ 20.0/( 1.0+exp((v+ 35.0)* 0.20000000000000001)));
        hRat = _pp_var_qt[tid_]/( 20.0+ 50.0/( 1.0+exp((v+ 50.0)* 0.14285714285714285)));
        a_0_ =  -1.0*mRat;
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        ica = _pp_var_gbar[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_h[tid_]*(v-eca);
        current_ = ica;
        conductivity_ = _pp_var_gbar[tid_]*_pp_var_m[tid_]*_pp_var_m[tid_]*_pp_var_h[tid_];
...
void mechanism_Ca_LVA_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_Ca_LVA_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Ca_LVA_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_Ca_LVA_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_Ca_LVA_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_Ca_LVA_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
