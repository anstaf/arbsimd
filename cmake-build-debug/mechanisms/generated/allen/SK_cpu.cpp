...
[[maybe_unused]] auto* _pp_var_zInf = pp->state_vars[1];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type l_ca;
        l_ca = cai;
        if (l_ca< 9.9999999999999995e-08) {
            l_ca = l_ca+ 9.9999999999999995e-08;
        }
        _pp_var_z[i_] =  1.0/( 1.0+pow( 0.00042999999999999999/l_ca,  4.7999999999999998));
    }
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type cai = _pp_var_ion_ca.internal_concentration[ion_ca_indexi_];
        arb_value_type b_0_, l_ca, a_0_, ll0_, ll1_;
        ll1_ =  0.;
        ll0_ =  0.;
        l_ca = cai;
        if (l_ca< 9.9999999999999995e-08) {
        ...
        _pp_var_zInf[i_] =  1.0/( 1.0+pow( 0.00042999999999999999/l_ca,  4.7999999999999998));
        a_0_ = _pp_var_zTau;
        b_0_ = _pp_var_zInf[i_];
        ll0_ =  -dt/a_0_;
        ll1_ = ( 1.0+ 0.5*ll0_)/( 1.0- 0.5*ll0_);
        _pp_var_z[i_] = b_0_+(_pp_var_z[i_]-b_0_)*ll1_;
    }
}
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type ik = 0;
        ik = _pp_var_gbar[i_]*_pp_var_z[i_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gbar[i_]*_pp_var_z[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_allen_catalogue_SK_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::allen_catalogue::kernel_SK::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::allen_catalogue::kernel_SK::min_align_;
    result.init_mechanism = arb::allen_catalogue::kernel_SK::init;
    result.compute_currents = arb::allen_catalogue::kernel_SK::compute_currents;
    result.apply_events = arb::allen_catalogue::kernel_SK::apply_events;
    result.advance_state = arb::allen_catalogue::kernel_SK::advance_state;
    result.write_ions = arb::allen_catalogue::kernel_SK::write_ions;
    result.post_event = arb::allen_catalogue::kernel_SK::post_event;
    return &result;
  }}
