...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        _pp_var_cai[i_] = _pp_var_minCai[i_];
    }
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        ba_0_ = ( -10000.0*ica*_pp_var_gamma[i_]/( 2.0*_pp_var_F*_pp_var_depth[i_])- -_pp_var_minCai[i_]/_pp_var_decay[i_])/a_0_;
        ll0_ = a_0_*dt;
...
extern "C" {
  arb_mechanism_interface* make_arb_allen_catalogue_CaDynamics_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::allen_catalogue::kernel_CaDynamics::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::allen_catalogue::kernel_CaDynamics::min_align_;
    result.init_mechanism = arb::allen_catalogue::kernel_CaDynamics::init;
    result.compute_currents = arb::allen_catalogue::kernel_CaDynamics::compute_currents;
    result.apply_events = arb::allen_catalogue::kernel_CaDynamics::apply_events;
    result.advance_state = arb::allen_catalogue::kernel_CaDynamics::advance_state;
    result.write_ions = arb::allen_catalogue::kernel_CaDynamics::write_ions;
    result.post_event = arb::allen_catalogue::kernel_CaDynamics::post_event;
    return &result;
  }}
