...
[[maybe_unused]] auto _pp_var_mTauF = pp->globals[1];\
[[maybe_unused]] auto _pp_var_hTauF = pp->globals[2];\
...
static void init(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        _pp_var_m[i_] =  1.0/( 1.0+exp( -(v+ 47.0-_pp_var_vshift)* 0.034482758620689655));
        _pp_var_h[i_] =  1.0/( 1.0+exp((v+ 66.0-_pp_var_vshift)* 0.10000000000000001));
    }
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type v = _pp_var_vec_v[node_indexi_];
        arb_value_type ik = 0;
        ik = _pp_var_gbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gbar[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_m[i_]*_pp_var_h[i_];
...
extern "C" {
  arb_mechanism_interface* make_arb_allen_catalogue_K_T_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::allen_catalogue::kernel_K_T::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::allen_catalogue::kernel_K_T::min_align_;
    result.init_mechanism = arb::allen_catalogue::kernel_K_T::init;
    result.compute_currents = arb::allen_catalogue::kernel_K_T::compute_currents;
    result.apply_events = arb::allen_catalogue::kernel_K_T::apply_events;
    result.advance_state = arb::allen_catalogue::kernel_K_T::advance_state;
    result.write_ions = arb::allen_catalogue::kernel_K_T::write_ions;
    result.post_event = arb::allen_catalogue::kernel_K_T::post_event;
    return &result;
  }}
