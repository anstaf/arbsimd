...
auto* _pp_var_zInf __attribute__((unused)) = params_.state_vars[1];\
...
void init(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_z[tid_] =  1.0/( 1.0+pow( 0.00042999999999999999/l_ca,  4.7999999999999998));
    }
}
...
void advance_state(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        _pp_var_zInf[tid_] =  1.0/( 1.0+pow( 0.00042999999999999999/l_ca,  4.7999999999999998));
        a_0_ = _pp_var_zTau;
        b_0_ = _pp_var_zInf[tid_];
        ll0_ =  -dt/a_0_;
        ll1_ = ( 1.0+ 0.5*ll0_)/( 1.0- 0.5*ll0_);
        _pp_var_z[tid_] = b_0_+(_pp_var_z[tid_]-b_0_)*ll1_;
    }
}
...
void compute_currents(arb_mechanism_ppack params_) {
    ...
    if (tid_<n_) {
        ...
        arb_value_type ik = 0;
        ik = _pp_var_gbar[tid_]*_pp_var_z[tid_]*(v-ek);
        current_ = ik;
        conductivity_ = _pp_var_gbar[tid_]*_pp_var_z[tid_];
...
void mechanism_SK_gpu_init_(arb_mechanism_ppack* p) {
    auto n = p->width;
...
void mechanism_SK_gpu_compute_currents_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_SK_gpu_advance_state_(arb_mechanism_ppack* p) {
    auto n = p->width;
    unsigned block_dim = 128;
    unsigned grid_dim = ::arb::gpu::impl::block_count(n, block_dim);
...
void mechanism_SK_gpu_write_ions_(arb_mechanism_ppack* p) {}

void mechanism_SK_gpu_post_event_(arb_mechanism_ppack* p) {}
void mechanism_SK_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* events) {}
